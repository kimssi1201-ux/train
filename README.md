...
- 호출량 보호를 위해 `전체` 화면은 6개 노선씩 순환 조회하고 자동 갱신하지 않습니다. 특정 노선 화면은 2분마다 갱신합니다.
- 브라우저는 실시간 위치 응답을 로컬에 저장해 새로고침 때 재사용하고, 하루 호출량이 너무 커지면 캐시 데이터만 표시합니다.
- Cloudflare Pages 함수는 서울 실시간 위치 정상 응답만 15분 캐시합니다. 한도 초과 같은 오류 응답은 캐시하지 않습니다.
